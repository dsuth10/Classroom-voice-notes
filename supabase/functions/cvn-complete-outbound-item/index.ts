...
serve(async (req: Request) => {
  ...
  if (!itemId || !leaseToken || !payloadHash || !contentHash) {
    return new Response(
      JSON.stringify({
        error: "missing_required_completion_fields",
        message: "item_id, lease_token, payload_hash, and content_hash are mandatory",
      }),
      {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
