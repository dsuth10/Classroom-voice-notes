...
serve(async (req: Request) => {
  ...
  if (
    ...
    payload.result_summary.length === 0 ||
    payload.result_summary.length > MAX_RESULT_SUMMARY_LENGTH
  ) {
    return new Response(
      `result_summary required, max ${MAX_RESULT_SUMMARY_LENGTH} chars`,
      {
        status: 400,
        headers: corsHeaders,
      },
  ...
  if (error) {
    console.error("cvn_complete_task RPC error:", error);
