...
//
// Deploy:   supabase functions deploy cvn-submit-task --no-verify-jwt
// Secrets:  set in Supabase Dashboard -> Edge Functions -> Secrets
//           CVN_HMAC_SECRET          — shared secret for HMAC-SHA256 signing
//           CVN_BEARER_TOKEN         — bearer token for Authorization header
//           SUPABASE_URL             — auto-set
//           SUPABASE_SERVICE_ROLE_KEY — auto-set
//
// Request:
//   POST /functions/v1/cvn-submit-task
//   Headers:
//     Authorization: Bearer <CVN_BEARER_TOKEN>
//     x-cvn-signature: <hex SHA-256 HMAC of raw body, using CVN_HMAC_SECRET>
//     Content-Type: application/json
//   Body: cvn.agent_task.v1 payload (see schema in validateSchema)

import { serve } from "https://deno.land/std@0.224.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*", // TODO: tighten in production
...
serve(async (req: Request) => {
  ...
  // 2. Read body once (for HMAC + JSON parse)
  ...
  if (ageSec > STALE_TIMESTAMP_SECONDS) {
  ...
  // 7. Atomic submit via RPC (task + event + queue in one transaction)
